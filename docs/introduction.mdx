...
| **Email** | `SendGmailOAuth`, `OAuthActions` | Gmail OAuth-backed sender. |
...
| **Auth / utilities** | `GetUserID`, OAuth helpers | Token issuance and account introspection. |
| **Audio files** | `CreateAudioFile`, `GetAudiosList` | Upload / list audio assets used by voice campaigns. |
| **Webhooks** | `GcpBudgetAlert`, `GoogleRISC` | Inbound notifications from cloud providers. |
