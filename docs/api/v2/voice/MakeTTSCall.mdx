---
title: "Place voice call (TTS)"
description: "Outbound voice call with TTS, IVR/DTMF, transfer, and recording."
api: "POST https://<base>/api/v2/MakeTTSCall.php"
---
...
<ParamField body="content" type="string">
  Audio file id from [`GetAudiosList`](/docs/api/v2/audio-files/GetAudiosList). Multiple slots available: `content1..content15`, plus `contentno`, `contentend`, `contentfail`, `contentdtmf`.
</ParamField>

<ParamField body="voice" type="string">
  TTS voice in the form `Provider|VoiceName`. Use [`GetTTSVoices`](/docs/api/v2/voice/GetTTSVoices) for the list of voices the account is allowed to use. ElevenLabs is **not** allowed in this endpoint.
</ParamField>
...
<ParamField body="ndig" type="integer">
  Number of digits expected in the response.
</ParamField>

<ParamField body="dtime" type="integer">
  Seconds to wait for digits.
</ParamField>

<ParamField body="dtmfconf" type="string">
...
<ParamField body="ringtime" type="integer">
...
<ParamField body="answer" type="integer">
...
<ParamField body="sms_dtmf" type="string">
...
<ParamField body="moh" type="integer">
...
<ResponseField name="return.status" type="string">
  `OK` on success; otherwise the failure message.
</ResponseField>

<ResponseField name="return.status_code" type="string">
  `0` on success; otherwise the numeric error code.
</ResponseField>

<ResponseField name="return.cd_id" type="string">
  Internal call id, prefixed with the customer database alias (`<cdcs_db>_<cd_id>`). Use it with [`GetCallStatus`](/docs/api/v2/voice/GetCallStatus) to query progress.
</ResponseField>

## Error codes

In addition to the [global authentication codes](/docs/api/v2/errors), `MakeTTSCall` may return:
...
- The `cd_id` returned has the form `<cdcs_db>_<numericId>`. Pass it as-is to [`GetCallStatus`](/docs/api/v2/voice/GetCallStatus).
