---
title: "Error codes"
description: "Numeric codes returned in the response status_code field."
---
...
When `status_code` is `0` and `status` is non-OK, the call left Velip but failed downstream. Check the `cdls_id` / `cd_id` returned in the response and consult [`GetCallStatus`](/docs/api/v2/voice/GetCallStatus) (for voice) or your delivery report dashboard.
