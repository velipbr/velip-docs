...
description: "Tokens (tsid), HTTP Basic, IP allowlists, and brute-force protection."
---
...
A `tsid` is a long-lived API token bound to a customer account and (optionally) a user. You can obtain one through the Velip web portal or by calling [`GetUserID`](/docs/api/v2/auth-token/GetUserID) with a valid `username` and `password`.
...
Used to authenticate by `username` / `password` directly — typically only when issuing a first token via [`GetUserID`](/docs/api/v2/auth-token/GetUserID).
...
See [Error codes](/docs/api/v2/errors) for the full table. The most common ones in this context:
