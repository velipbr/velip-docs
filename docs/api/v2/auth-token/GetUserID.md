...
- **`return.aut_app`** (*string*) — `1` if the user can use the API, `0` otherwise.


- **`return.aut_send`** (*string*) — `1` if the user can call send-message endpoints (`MakeSMS`, `MakeWhatsapp`, `MakeTTSCall`).


- **`return.aut_camp`** (*string*) — `1` if the user can manage campaigns (`CreateCampaign`, `ChangeCampaign`).


- **`return.tsid`** (*string*) — Newly issued token (20 chars) when `tsid=1` was passed; `null` when only introspecting.
