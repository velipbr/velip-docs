...
Token authentication required. See [Authentication](/docs/api/v2/authentication). Additionally, the `from_email` must already be linked to your customer through the Velip portal's Gmail OAuth onboarding flow (records stored in `cd_oauth_connections` + `cd_oauth_providers`).
