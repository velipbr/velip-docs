...
description: "Update schedule, throughput, or status of an existing campaign."
api: "POST https://<base>/api/v2/ChangeCampaign.php"
---
...
The endpoint applies all provided fields atomically and returns the resulting campaign in the same shape as [`GetCampaignsList`](/docs/api/v2/campaigns/GetCampaignsList).
...
<ParamField body="cp_id" type="integer" required>
  Campaign id (`cd_programa.cp_id`).
</ParamField>

<ParamField body="active" type="integer">
  `0` to pause, `1` to activate.
</ParamField>

<ParamField body="lig_min" type="string">
  Calls-per-minute throttle. Either a non-negative integer or `max`.
</ParamField>

<ParamField body="pas" type="integer">
