---
title: "Create contact-center queue"
description: "Provision a new queue (voice or chat) used to route conversations to operators."
api: "POST https://<base>/api/v2/CreateCenterQueue.php"
---
...
After creation you can attach the queue to a [`CreateCampaign`](/docs/api/v2/campaigns/CreateCampaign) request via `queue_id`.
...
<ParamField body="ai_tutor_id" type="integer">
  ID of a `cc_bot_agent` (active) used as a tutor for human operators.
</ParamField>

<ParamField body="ai_chatbot_id" type="integer">
...
<ParamField body="button_1, button_2, ... button_8" type="string">
  Label for the corresponding quick action button.
</ParamField>

### Dropdowns

<ParamField body="dropdown_1" type="string | string[]">
  Options for the first dropdown shown to operators. Accepts a `;` / `,` / newline-separated string or a JSON array.
</ParamField>

<ParamField body="dropdown_2" type="string | string[]">
  Same as above, for the second dropdown.
</ParamField>

### Knowledge base templates

<ParamField body="templates" type="string | integer[]">
